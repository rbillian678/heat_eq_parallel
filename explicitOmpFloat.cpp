...
namespace data
{
    constexpr float T{0.1}, dx{1.0 / 512}, dy{1.0 / 512}, dt{8e-7}, alpha{1};
    ...
    const float dtOverDxSq = data::dt / (data::dx * data::dx);
    const float dtOverDySq = data::dt / (data::dy * data::dy);
}
// u(x, y, 0) = sin(pi* x) * sin(pi * y)
float timeLowerBoundary(int x, int y)
{
...
// L^2 error
float computeError(const std::vector<std::vector<float>> &uNumeric)
{
    float ans = 0;
...
#pragma omp simd
        ...
        {
            float diff = calculateExactSol(i, j) - uNumeric[i][j];
...
void explicitOmp()
{
    ...
    // u(x, y, m*dt)
    std::vector<std::vector<float>> uXyPrev(data::numXSteps, std::vector<float>(data::numYSteps, 0));
...
#pragma omp simd
    ...
    // u(x, y, m+1*dt)
    std::vector<std::vector<float>> uXyCurr(data::numXSteps, std::vector<float>(data::numYSteps, 0));
